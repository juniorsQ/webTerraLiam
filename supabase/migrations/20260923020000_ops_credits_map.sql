-- Map pins, provider usage, and parent fallback for characters without created_by.
...
begin
  ...
  select coalesce((value ->> 'unit_cost_usd')::numeric, 0.28)
  into v_unit
  from public.platform_settings
  where key = 'video_policy';
  v_unit := coalesce(v_unit, 0.28);
  ...
  select jsonb_build_object(
    ...
    'providers', jsonb_build_object(
      'fal', jsonb_build_object(
        ...
        'videos', (select count(*) from public.pois where anim_video_url is not null),
        'jobs', (select count(*) from public.video_generation_jobs),
        'job_spend_usd', (
          select coalesce(sum(cost_usd_est), 0)
          from public.video_generation_jobs
          where status in ('queued', 'processing', 'completed')
  ...
  if p_resource = 'users' then
    ...
    from (
      select
        coalesce(w.name, 'Mundo sin nombre') as mundo,
        coalesce(poi.title, 'Personaje sin nombre') as personaje,
        coalesce(nullif(p.display_name, ''), 'Creador del mundo') as solicitado_por,
        j.status as estado,
